 ...
 *
 * Steps:  postal = 0, discovery = 1, stores = 2, flyers = 3, prefs = 4.
 *
 ...
 *  - Step 3 (flyers, bring-your-own-flyer upload) and step 4 (prefs) share the
 *    same gate: step 2's condition holds AND the persisted prefs already carry
 *    at least one selected store (the stores screen keeps its selection in local
 *    state and only persists it on Continue — so we gate on the persisted value,
 *    not transient UI state). Uploading a flyer is OPTIONAL, so prefs is NOT
 *    gated on whether any flyer was uploaded.
 */
...
export const ONBOARDING_STEP_ROUTE: Record<number, string> = {
  ...
  [ONBOARDING_STEP.flyers]: '/onboarding/flyers',
